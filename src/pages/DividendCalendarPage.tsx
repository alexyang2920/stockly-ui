  ...
  return <main className="page-shell">
    ...
    {portfolios.length === 0 && !loading ? <section className="rounded-[22px] border border-dashed border-[#b9cce0] bg-white py-20 text-center"><h2 className="text-xl font-semibold">No portfolio selected</h2><p className="mt-2 text-sm text-[#526b84]">Create a portfolio and add a position to start planning dividend income.</p></section> : <>
      ...
      <section className="surface-card surface-card--flush">
        <div className="flex items-center justify-between gap-2 border-b border-[#dbe6f2] px-3 py-4 sm:px-6">
          <button disabled={monthOffset <= 0} onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))} className="grid size-10 shrink-0 place-items-center rounded-xl border border-[#c4d5e8] hover:bg-[#f4f8fd] disabled:cursor-not-allowed disabled:opacity-30" aria-label="Previous month">←</button>
          <div className="min-w-0 flex-1 text-center"><div className="flex flex-col items-center gap-1 sm:flex-row sm:justify-center sm:gap-2"><h2 className="text-base font-semibold leading-tight tracking-[-.02em] sm:text-lg">{title}</h2><span className="shrink-0 rounded-lg bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700">+{money(monthlyTotal, selected?.currency)}</span></div><p className="mt-1 truncate text-[11px] text-[#526b84]">Announced payments · projected distributions</p></div>
          <button disabled={monthOffset >= 11} onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))} className="grid size-10 shrink-0 place-items-center rounded-xl border border-[#c4d5e8] hover:bg-[#f4f8fd] disabled:cursor-not-allowed disabled:opacity-30" aria-label="Next month">→</button>
        </div>
...
function AnnualIncomeOverview({ total, currency, marketValue, months, monthlyIncome, monthlyBreakdown, scheduled, sources, events }: { total: number, currency?: string, marketValue: number, months: Date[], monthlyIncome: number[], monthlyBreakdown: { scheduled: number, estimated: number }[], scheduled: number, sources: number, events: DividendCalendarEvent[] }) {
  ...
  return <section className="mb-5 grid grid-cols-[minmax(0,1fr)] gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
    ...
    <div className="space-y-5"><div className="rounded-[22px] border border-[#c4d5e8] bg-white p-5 sm:p-6"><div className="mb-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><h2 className="font-semibold tracking-[-.02em]">12-month income forecast</h2><p className="mt-1 text-xs text-[#526b84]">Scheduled payments and estimates by month</p></div><div className="flex flex-wrap gap-3 text-[10px] font-semibold text-[#506981]"><Legend color="bg-[#0b5b9e]" label="Scheduled" /><Legend color="bg-amber-400" label="Estimated" /></div></div><div className="overflow-x-auto pb-1"><div className="flex h-44 min-w-[540px] items-end gap-2 sm:min-w-0 sm:gap-3">{months.map((forecastMonth, index) => <div key={dateKey(forecastMonth)} className="group relative flex h-full min-w-0 flex-1 flex-col justify-end text-center"><BarTooltip month={forecastMonth} total={monthlyIncome[index]} amounts={monthlyBreakdown[index]} currency={currency} /><span className="mb-1 truncate text-[9px] font-semibold text-[#506a84]">{monthlyIncome[index] ? compactMoney(monthlyIncome[index], currency) : ''}</span><div className="mx-auto flex w-full max-w-10 flex-col-reverse overflow-hidden rounded-t-md bg-[#e6f0fb]" style={{ height: `${Math.max(monthlyIncome[index] ? monthlyIncome[index] / maximum * 100 : 2, 2)}%` }}><BarSegment value={monthlyBreakdown[index].scheduled} total={monthlyIncome[index]} color="bg-[#0b5b9e]" /><BarSegment value={monthlyBreakdown[index].estimated} total={monthlyIncome[index]} color="bg-amber-400" /></div><span className="mt-2 text-[9px] text-[#526b84]">{new Intl.DateTimeFormat('en-US', { month: 'short' }).format(forecastMonth)}</span></div>)}</div></div></div><IncomeReliability events={events} total={total} currency={currency} /></div>
  </section>
}
...
function EventRow({ event, onSelect }: { event: DividendCalendarEvent, onSelect: () => void }) {
  return <button onClick={onSelect} className="flex w-full items-center gap-2 px-3 py-4 text-left sm:gap-3 sm:px-4"><div className="w-9 shrink-0 text-center sm:w-11"><strong className="block text-lg">{marketDateDay(event.calendarDate)}</strong><span className="text-[10px] uppercase text-[#526b84]">{formatMarketDate(event.calendarDate, { weekday: 'short' })}</span></div><InstrumentMark symbol={event.symbol} size="small" /><div className="min-w-0 flex-1"><strong className="block text-sm leading-tight">{event.symbol}</strong><span className="mt-0.5 block text-[10px] font-bold uppercase leading-tight text-[#526b84]">{event.estimated ? 'Estimated' : event.dateType === 'PAY_DATE' ? 'Pay date' : 'Ex-date'}</span><p className="mt-1 truncate text-xs text-[#526b84]">{event.name} · Ex {shortDate(event.exDividendDate)}</p></div><div className="min-w-16 shrink-0 text-right"><strong className="block text-sm">{money(event.projectedAmount, event.currency)}</strong><span className="whitespace-nowrap text-[10px] text-[#526b84]">{event.quantity} × {money(event.amountPerShare, event.currency)}</span></div></button>
}
