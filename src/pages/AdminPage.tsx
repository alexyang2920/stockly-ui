...
function AdminPage({ auth, onNeedAuth }: { auth: AuthResponse | null, onNeedAuth: () => void }) {
  ...
  const runSync = async (resumeDividends = false) => {
    if (!resumeDividends && !Object.values(selected).some(Boolean)) { setError('Select at least one dataset.'); return }
    setSyncing(true); setError(''); setSummary('')
    try {
      const result = await syncMarketData(auth, {
        quotes: resumeDividends ? false : selected.QUOTES,
        dividends: resumeDividends || selected.DIVIDENDS,
        splits: resumeDividends ? false : selected.SPLITS,
        marketDate: !resumeDividends && selected.QUOTES ? marketDate : undefined,
        corporateActionsFrom: resumeDividends ? undefined : corporateFrom || undefined,
      })
