...
function AdminPage({ auth, onNeedAuth }: { auth: AuthResponse | null, onNeedAuth: () => void }) {
  ...
  return <main className="mx-auto max-w-[1560px] px-5 py-8 lg:px-8 lg:py-11">
    ...
    <nav className="mb-7 flex gap-1 rounded-2xl border border-[#dfe4df] bg-white p-1.5" aria-label="Administration sections"><button onClick={() => { setTab('market'); setError('') }} className={`flex-1 rounded-xl px-3 py-3 text-sm font-bold transition sm:flex-none sm:px-4 ${tab === 'market' ? 'bg-[#173c2c] text-white' : 'text-[#6e7a73] hover:bg-[#f3f5f2]'}`}>Market Data</button><button onClick={() => { setTab('instruments'); setError('') }} className={`flex-1 rounded-xl px-3 py-3 text-sm font-bold transition sm:flex-none sm:px-4 ${tab === 'instruments' ? 'bg-[#173c2c] text-white' : 'text-[#6e7a73] hover:bg-[#f3f5f2]'}`}>Instruments</button><button onClick={() => { setTab('financial'); setError('') }} className={`flex-1 rounded-xl px-3 py-3 text-sm font-bold transition sm:flex-none sm:px-4 ${tab === 'financial' ? 'bg-[#173c2c] text-white' : 'text-[#6e7a73] hover:bg-[#f3f5f2]'}`}>Financial Data</button></nav>
