...
function AdminPage({ auth, onNeedAuth }: { auth: AuthResponse | null, onNeedAuth: () => void }) {
  ...
  return <main className="mx-auto max-w-[1560px] px-5 py-8 lg:px-8 lg:py-11">
    ...
    {tab === 'financial' && <section className="max-w-2xl rounded-[22px] border border-[#c4d5e8] bg-white p-5 md:p-7"><div className="flex items-start gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-[#e4effb] text-[#0b5b9e]"><svg className="size-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M5 20V10M12 20V4M19 20v-7M3 20h18" /></svg></span><div><h2 className="text-xl font-semibold tracking-[-.025em]">Company financial data</h2><p className="mt-1 text-sm leading-6 text-[#556c84]">Select an instrument and import its latest SEC company facts, including revisions to previously filed periods.</p></div></div><div className="mt-7"><label htmlFor="financial-symbol" className="text-sm font-semibold">Instrument</label><div className="relative mt-2"><input id="financial-symbol" list="financial-instruments" value={financialSymbol} onChange={(event) => { setFinancialSymbol(event.target.value.toUpperCase().slice(0, 50)); setFinancialSummary('') }} placeholder="Search by symbol or company name" autoComplete="off" className="w-full rounded-xl border border-[#c3d5e8] bg-white px-3.5 py-3 text-sm outline-none focus:border-[#327ab7] focus:ring-4 focus:ring-[#d9e7f5]" /><datalist id="financial-instruments">{financialSuggestions.map((instrument) => <option key={instrument.symbol} value={instrument.symbol}>{instrument.name} · {instrument.exchange}</option>)}</datalist></div><button onClick={syncSelectedFinancials} disabled={financialSyncing || !financialSymbol.trim()} className="mt-4 w-full rounded-xl bg-[#0b3b66] px-5 py-3.5 text-sm font-bold text-white hover:bg-[#0b4f89] disabled:cursor-wait disabled:opacity-60">{financialSyncing ? 'Synchronizing financial data…' : 'Sync financial data'}</button></div><div className="mt-6 rounded-xl bg-[#f4f8fd] p-4 text-xs leading-5 text-[#4d6882]">The synchronization upserts SEC facts by filing context, so amended filings can correct previously stored periods without creating duplicate values.</div></section>}
