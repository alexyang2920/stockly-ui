...
type HoldingSortKey = 'instrument' | 'quantity' | 'averageCost' | 'marketPrice' | 'marketValue' | 'estimatedDividendIncome' | 'unrealizedGain' | 'realizedGain'
...
function PortfolioPage({ auth, section, requestedPortfolioId, onNeedAuth, onSelectInstrument, startWithTransaction = false }: PortfolioPageProps) {
  ...
  const sortedHoldings = useMemo(() => [...holdings].sort((left, right) => {
    const values: Record<HoldingSortKey, [string | number | null, string | number | null]> = {
      ...
      estimatedDividendIncome: [left.estimatedDividendIncome, right.estimatedDividendIncome],
  ...
  return <main className="page-shell">
    <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end">
      ...
      {selected && <div className="flex w-full flex-col gap-2 self-start sm:w-auto sm:flex-row md:self-auto"><button onClick={() => { setEditing(null); setShowTransaction(true) }} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#38bdf8] px-4 py-3 text-sm font-bold text-[#162b45] hover:bg-[#22b4ee] sm:w-auto"><Icon className="size-4"><path d="M12 5v14M5 12h14" /></Icon>Add transaction</button></div>}
    </div>
