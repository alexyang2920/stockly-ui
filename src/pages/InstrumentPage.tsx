...
function FinancialMetricCell({ label, description, periods, factsByPeriod, rowIndex, rowCount, expanded, onToggleChanges }: { label: string, description: string, periods: FinancialPeriodColumn[], factsByPeriod?: Map<string, FinancialFact>, rowIndex: number, rowCount: number, expanded: boolean, onToggleChanges: () => void }) {
  ...
  return <th onClick={onToggleChanges} onMouseLeave={() => setTrendOpen(false)} className="group sticky left-0 z-20 cursor-pointer border-r border-[#e4e8e4] bg-white px-7 py-4 text-sm font-semibold whitespace-nowrap dark:bg-[#141d19] dark:text-[#e8eee9]" aria-expanded={expanded} title="Show year-over-year changes">
    <span className="flex w-full items-center justify-between gap-4 text-left">
      <span>{label}</span>
      <button type="button" onClick={(event) => { event.stopPropagation(); setTrendOpen((open) => !open) }} className="-m-2 grid size-8 shrink-0 place-items-center rounded-lg outline-none transition hover:bg-[#eef2ee] focus-visible:ring-2 focus-visible:ring-[#789887] dark:hover:bg-[#203028]" aria-label={`${trendOpen ? 'Hide' : 'Show'} trend and explanation for ${label}`} aria-expanded={trendOpen}>
