...
const holdingGroup = (holding: Holding) => holding.instrumentType === 'ETF' ? 'Funds' : holding.sector || holding.category || holding.instrumentType
...
function OverviewPage({ auth, portfolioId, onNeedAuth, onOpenHoldings, onOpenDividends, onSelectInstrument }: { auth: AuthResponse | null, portfolioId?: string, onNeedAuth: () => void, onOpenHoldings: () => void, onOpenDividends: () => void, onSelectInstrument: (symbol: string) => void }) {
  ...
  return <main className="mx-auto max-w-[1380px] px-5 py-8 lg:px-8 lg:py-11"><div className="mb-8"><p className="text-xs font-bold uppercase tracking-[.15em] text-[#718078]">Portfolio overview</p><h1 className="mt-2 text-[36px] font-semibold tracking-[-.045em] md:text-[46px]">{portfolio?.name ?? 'Overview'}</h1><p className="mt-2 text-sm text-[#738078]">Performance, income, and diversification in one place.</p></div>{error && <div className="mb-5 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700">{error}</div>}{!portfolio ? <div className="rounded-[22px] border border-dashed border-[#cfd7d1] bg-white py-20 text-center"><h2 className="text-xl font-semibold">Create your first portfolio</h2></div> : <>
    ...
    {concentration && concentration.percent >= 40 && <section className="mb-5 flex flex-col justify-between gap-4 rounded-[20px] border border-amber-200 bg-amber-50 p-5 sm:flex-row sm:items-center"><div><p className="text-[10px] font-bold uppercase tracking-[.13em] text-amber-700">Diversification insight</p><h2 className="mt-1 font-semibold">{concentration.name} represents {concentration.percent.toFixed(1)}% of this portfolio</h2><p className="mt-1 text-xs text-[#7b867f]">A concentrated allocation can make performance more sensitive to one part of the market.</p></div><button onClick={onOpenHoldings} className="shrink-0 rounded-xl bg-[#173c2c] px-4 py-2.5 text-xs font-bold text-white">Review holdings</button></section>}
