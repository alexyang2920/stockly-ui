...
function PortfolioNav({ auth, selectedId, onSelect, mobile = false }: PortfolioNavProps) {
  ...
  return <>
    <div ref={menuRef} className={`relative ${mobile ? 'mb-3 block w-full' : 'hidden md:block'}`}>
      <button disabled={loading} onClick={() => setMenuOpen((open) => !open)} className={`${mobile ? 'w-full' : 'w-36 xl:w-44'} flex h-10 items-center gap-2 rounded-xl border border-[#dfe4df] bg-white px-3 text-left transition hover:border-[#aeb9b1] disabled:opacity-60`} aria-haspopup="menu" aria-expanded={menuOpen}>
      ...
      {menuOpen && <div role="menu" className={`${mobile ? 'left-0 right-0' : 'right-0 w-60'} absolute top-[calc(100%+8px)] z-50 overflow-hidden rounded-2xl border border-[#dce3dd] bg-white shadow-[0_18px_50px_rgba(20,38,29,.2)] dark:border-[#35463d] dark:bg-[#18231e]`}>
