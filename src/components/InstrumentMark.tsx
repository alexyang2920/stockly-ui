...
function InstrumentMark({ symbol, size = 'medium' }: { symbol: string, size?: 'small' | 'medium' | 'large' }) {
  ...
  return <div className={`${sizeClass} ${colors[symbol] ?? 'bg-[#0b3b66] text-white'} grid shrink-0 place-items-center font-extrabold tracking-tight shadow-sm`}>{symbol === 'AAPL' ? '●' : symbol.slice(0, 2)}</div>
}
